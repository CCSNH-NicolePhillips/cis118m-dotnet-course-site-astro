...
export const TUTOR_CONTEXTS: Record<string, string> = {
  ...
  // Week 6
  'week-06': "Week 6: Decision Structures (if/else). Students learn boolean logic and program flow control. Topics: bool type, comparison operators (==, !=, <, >, <=, >=), if/else statements with K&R brace style, logical operators (&&, ||, !), else if chains, short-circuit evaluation. Metaphors: Boolean Gate (binary switch), Binary Branch (fork in the road), Circuit Filter (logic gates), Control Tower (multi-path routing). IMPORTANT: Professor is traveling with limited connectivity this week — be extra proactive and helpful. Always use K&R (Egyptian) brace style in examples. camelCase for variables, UPPER_CASE for constants.",
