/**
 * List Classic Quizzes via Canvas API.
 *
 * Usage:
 *   node scripts/canvas-classic-quizzes-list.mjs --match "Week 9"
 *
...
async function listAllClassicQuizzes({ base, courseId, token }) {
  const results = [];
  let url = `${base}/api/v1/courses/${encodeURIComponent(courseId)}/quizzes?per_page=100`;

  while (url) {
    const { res, json } = await canvasFetch(url, { token });
    if (Array.isArray(json?.quizzes)) {
      results.push(...json.quizzes);
    } else if (Array.isArray(json)) {
      // some Canvas instances return an array directly
      results.push(...json);
    }
...
async function main() {
  ...
  const quizzes = await listAllClassicQuizzes({ base: args.base, courseId: args.course, token: args.token });
  ...
  console.log(`Classic Quizzes matched: ${filtered.length}${re ? ` (filter: ${args.match})` : ''}`);
