...
export const TUTOR_CONTEXTS = {
  ...
  'week-10-lab': "Week 10 Lab: The Modular System — Student Report Generator. Students define these exact method signatures: static void PrintReportHeader(), static void PrintSeparator(), static void PrintStudentRecord(string name, int grade), static void PrintReportFooter(int studentCount). Main uses a for loop (3 iterations) to collect names and grades, then calls PrintStudentRecord each time. PrintStudentRecord includes letter grade logic (A: 90-100, B: 80-89, C: 70-79, D: 60-69, F: below 60). Main must NOT contain any Console.WriteLine formatting — all display logic must be in methods. DRY: PrintSeparator is called multiple times — not copy-pasted.",
